...
class Divi_Login_BY_ZEN {

public function inject_zencaptcha_widget_divi_login($output, $slug){
    ...
    if (isset($_GET['error_message'])) {
        ...
        $errorHTML = "<div class='error-message' style='color: red; background-color: white; border-radius: 10px; padding: 10px;margin-bottom:10px;'>" . $error_message . "</div>
        <script>
    ...
    $capture    = '/(<p>[\s]*?<button)/';
    $newHTML = $errorHTML.zencaptcha_insert_html_widget($zencaptchainstance) . "<style>.zenc-captcha{margin-bottom:10px;}</style> \n" . '$1';
    return preg_replace($capture, $newHTML, $output);
}


public function verify_zencaptcha_divi_login( $user, string $password ) {
...
public function __construct() {
    $zencaptchainstance = ZENCAPTCHA_MAIN::$instance;
        if (!$zencaptchainstance->are_keys_set() || !get_option('zen-divilogin-activate')) {
            return;
        }

    add_filter( 'et_pb_login_shortcode_output', [ $this, 'inject_zencaptcha_widget_divi_login' ], 10, 2 );
    add_filter( 'wp_authenticate_user', [ $this, 'verify_zencaptcha_divi_login' ], 10, 2 );
}

}

$zencaptchaPlugin = new Divi_Login_BY_ZEN();