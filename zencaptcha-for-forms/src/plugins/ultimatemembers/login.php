...
class UM_FORMS_LOGIN_BY_ZEN{

    public function inject_zencaptcha_widget_um_login() {
    ...
    public function verify_zencaptcha_um_login(array $submitted_data, array $form_data = []){
    ...
    public function __construct() {
        $zencaptchainstance = ZENCAPTCHA_MAIN::$instance;
        if (!$zencaptchainstance->are_keys_set() || !get_option('zen-ultimatememberlogin-activate')) {
            return;
        }

        add_filter( 'um_after_login_fields', [ $this, 'inject_zencaptcha_widget_um_login' ], 100 );
        add_action( 'um_submit_form_errors_hook_login', [ $this, 'verify_zencaptcha_um_login' ], 10, 2 );
...
$zencaptchaPlugin = new UM_FORMS_LOGIN_BY_ZEN();
