...
class UM_FORMS_RESET_BY_ZEN{

    public function inject_zencaptcha_widget_um_reset() {
        ...
        if($error_message!=""){
            ...
            echo zencaptcha_insert_html_widget($zencaptchainstance).'<style>.zenc-captcha{margin-top:10px;margin-bottom:10px;}</style>';
        }
		
    }
    
    public function verify_zencaptcha_um_reset(array $submitted_data, array $form_data = []){
    ...
    public function __construct() {
        $zencaptchainstance = ZENCAPTCHA_MAIN::$instance;
        if (!$zencaptchainstance->are_keys_set() || !get_option('zen-ultimatememberresetpass-activate')) {
            return;
        }

        add_filter( 'um_after_password_reset_fields', [ $this, 'inject_zencaptcha_widget_um_reset' ], 100 );
        add_action( 'um_reset_password_errors_hook', [ $this, 'verify_zencaptcha_um_reset' ], 10, 2 );
...
$zencaptchaPlugin = new UM_FORMS_RESET_BY_ZEN();
