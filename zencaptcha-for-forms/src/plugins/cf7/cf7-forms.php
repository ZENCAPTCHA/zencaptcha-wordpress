...
function wpcf7_dev_display_zencaptcha($formelements){
    $cf7_script_version = '0.0.22';

    $zencaptchainstance = ZENCAPTCHA_MAIN::$instance;
    if (!$zencaptchainstance->are_keys_set() || !get_option('zen-cf7-activate')) {
        return $formelements;
    }

    wp_enqueue_script(
        'zencaptcha-cf7-script-enqueue', 
        zencap_plugin_url('src/plugins/cf7') . 'script.js', 
        array(), 
        $cf7_script_version, 
        true
    );
    
    return preg_match('/<div.*class=".*zenc-label-container.*".*<\/div>/', $formelements) ? $formelements : $formelements . zencaptcha_insert_html_widget($zencaptchainstance);
}



function wpcf7_spam_check_by_zencaptcha($spam, $submission){
    ...
    $verifyemail = get_option('zen-cf7-verifyemail') ? $email_value : "";
...
add_filter('wpcf7_form_elements', 'wpcf7_dev_display_zencaptcha', 100, 1);
add_filter('wpcf7_spam', 'wpcf7_spam_check_by_zencaptcha', 100, 2);
