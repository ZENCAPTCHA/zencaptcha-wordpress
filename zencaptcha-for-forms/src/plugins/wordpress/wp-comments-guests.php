...
function zen_wp_comment_after(){
    ...
    if (is_user_logged_in() && get_option('zen-wp-comments-loggedin')) {
        ZENCAPTCHA_MAIN::$skipcaptcha=true;
        echo zencaptcha_insert_html_widget($zencaptchainstance);
    }
    else if (!is_user_logged_in() && get_option('zen-wp-comments-guests')) {
        ZENCAPTCHA_MAIN::$skipcaptcha=true;
        echo zencaptcha_insert_html_widget($zencaptchainstance);
    }
    ...
    if (isset($_GET['error_message'])) {
        ...
        echo "<div class='error-message' style='color: red; background-color: white; border-radius: 10px; padding: 10px;margin-bottom:10px;font-size:16px;'>" . $error_message . "</div>
        <script>
...
function zen_wp_comment_post($data){
    ...
    if ($data["user_id"] == 0 && !get_option('zen-wp-comments-guests')) {
        return $data;
    }
    else if ($data["user_id"] != 0 && !get_option('zen-wp-comments-loggedin')) {
        //not check email
        return $data;
    }
    ...
    $verifyemail = get_option('zen-wp-verifyemail') ? $posteremail : "";
...
add_action( 'comment_form_after_fields', 'zen_wp_comment_after', 20, 0 );
add_action( 'comment_form_logged_in_after', 'zen_wp_comment_after', 20, 0 );
add_filter( 'preprocess_comment', 'zen_wp_comment_post', 10, 1 );	