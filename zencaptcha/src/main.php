...
class ZENCAPTCHA_MAIN
{
    ...
    public static $version=ZENCA_VERSION;
    public static $zencaptcha_sitekey = "zenca_sitekey";
    public static $zencaptcha_secret_key = "zenca_secret_key";
    public static $zencaptcha_widget_lang = "zenca_widget_lang";
    ...
    public static function translate_zen($key)
    {
        ...
        if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
            // Get user languages from the header
            $user_langs = isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) ? explode(',', $_SERVER['HTTP_ACCEPT_LANGUAGE']) : array();
            
            $preferred_lang = isset($user_langs[0]) ? sanitize_text_field(strtok($user_langs[0], ';')) : '';

            $preferred_lang_code = substr($preferred_lang, 0, 2);

            /*v1:$user_langs = explode(',', $_SERVER['HTTP_ACCEPT_LANGUAGE']);
    ...
    public function init()
    {
        ...
        register_activation_hook(ZENCA_MAIN_FILE, array($this, 'activate_zencaptcha_plugin'));
        register_uninstall_hook(ZENCA_MAIN_FILE, array($this, 'uninstall_zencaptcha_plugin'));
        register_deactivation_hook(ZENCA_MAIN_FILE, array($this, 'deactivation_zencaptcha_plugin'));

        add_action('admin_menu', array($this, 'menu'));
        add_action('admin_init', array($this, 'settings'));

        add_action( 'admin_notices', array($this, 'zenca_admin_notice_no_keys'), 999 );

        add_action('admin_enqueue_scripts', array($this, 'enqueue_admin_styles'));

        add_filter('plugin_action_links_'.ZENCA_DOMAIN.'/'.ZENCA_MAIN_PLUGIN_PHP, array($this, 'zenca_plugin_action_links'), 10, 2);

    }

    public static function zenca_login_data_errors_Alert(){
        return self::translate_zen('<strong>Error.</strong> Your login data contains errors.');
    }

    public static function zenca_failed_try_again_Alert() {
        return self::translate_zen('Please confirm that you are human. <strong>Try again.</strong>');
    }
    public static function zenca_failed_try_again_Alert_no_html() {
        return self::translate_zen('Please confirm that you are human. Try again.');
    }

    public static function zenca_check_failed_Alert() {
        return self::translate_zen('Robot check failed, please retry.');
    }
    public static function zenca_check_failed_Alert_no_html() {
        return self::translate_zen('Robot check failed, please retry.');
    }

    public static function zenca_check_again_Alert(){
        return self::translate_zen('Confirm the robot check again.');
    }
    public static function zenca_check_again_Alert_no_html(){
        return self::translate_zen('Confirm the robot check again.');
    }
    
    public static function zenca_use_valid_email_Alert() {
        return self::translate_zen('Use a valid email address.');
    }
    public static function zenca_use_valid_email_Alert_no_html() {
        return self::translate_zen('Use a valid email address.');
    }

    public static function zenca_no_access(){
        return self::translate_zen('<b>Your access has been blocked.</b>');
    }
    public static function zenca_no_access_no_html(){
        return self::translate_zen('Your access has been blocked.');
    }

    public static function zenca_fill_in_all_fields_Alert(){
        return self::translate_zen('<b>Error:</b> Please fill in the required fields.');
    }
    public static function zenca_fill_in_all_fields_Alert_no_html(){
        return self::translate_zen('Error: Please fill in the required fields.');
    }

    public function enqueue_admin_styles() {
        wp_enqueue_style('zencaptcha-admin-css', zenca_plugin_url('assets/css/admin-styles.css'), array(), ZENCA_CSS_VERSION);
    }
    ...
    public function loadTextDomain()
    {
        load_plugin_textdomain(
			'zencaptcha',
			false,
			dirname( plugin_basename( ZENCA_FILE ) ) . '/languages/'
		);
    }
    ...
    public function activate_zencaptcha_plugin()
    {
        ...
        if (!get_option('zenca_country_blacklist')) {
    ...
    public static function uninstall_zencaptcha_plugin()
    {
        ...
        delete_option('zenca-wp-login');
        delete_option('zenca-wp-register');
        delete_option('zenca-wp-resetpass');
        delete_option('zenca-wp-comments-guests');
        delete_option('zenca-wp-comments-loggedin');

        delete_option('zenca-woo-login');
        delete_option('zenca-woo-register');
        delete_option('zenca-woo-resetpass');
        delete_option('zenca-woo-checkout');

        delete_option('zenca-elementor-activate');

        delete_option('zenca-wpforms-activate');

        delete_option('zenca-cf7-activate');
        delete_option('zenca-gravity-activate');
        //delete_option('zen-ninja-activate');
        delete_option('zenca-mailchimp-activate');

        delete_option('zenca-divilogin-activate');
        delete_option('zenca-divicomment-activate');
        delete_option('zenca-diviemailoptin-activate');
        delete_option('zenca-divicontact-activate');

        delete_option('zenca-htmlforms-activate');

        delete_option('zenca-avada-activate');
        delete_option('zenca-forminator-activate');
        delete_option('zenca-fluentform-activate');

        delete_option('zenca-profilebuilderlogin-activate');
        delete_option('zenca-profilebuilderregister-activate');
        delete_option('zenca-profilebuilderresetpass-activate');

        delete_option('zenca-ultimatememberlogin-activate');
        delete_option('zenca-ultimatememberregister-activate');
        delete_option('zenca-ultimatememberresetpass-activate');

        delete_option('zenca-wp-verifyemail');
        delete_option('zenca-woo-verifyemail');
        delete_option('zenca-elementor-verifyemail');
        delete_option('zenca-wpforms-verifyemail');
        delete_option('zenca-cf7-verifyemail');
        delete_option('zenca-gravity-verifyemail');
        //delete_option('zen-ninja-verifyemail');
        delete_option('zenca-mailchimp-verifyemail');
        delete_option('zenca-divi-verifyemail');
        delete_option('zenca-avada-verifyemail');
        delete_option('zenca-htmlforms-verifyemail');
        delete_option('zenca-forminator-verifyemail');
        delete_option('zenca-fluentform-verifyemail');
        delete_option('zenca-profilebuilder-verifyemail');
        delete_option('zenca-ultimatemember-verifyemail');
    ...
    {
        add_options_page(
            __('Zencaptcha Settings', 'zencaptcha'),
            'Zencaptcha',
            'manage_options',
            'zencaptcha-admin',
    ...
    public function zenca_plugin_action_links($links)
    {
    ...
    public function settings()
    {
        ...
        register_setting('zencaptcha-settings-group', 'zenca-wp-login');
        register_setting('zencaptcha-settings-group', 'zenca-wp-register');
        register_setting('zencaptcha-settings-group', 'zenca-wp-resetpass');
        register_setting('zencaptcha-settings-group', 'zenca-wp-comments-guests');
        register_setting('zencaptcha-settings-group', 'zenca-wp-comments-loggedin');
        
        register_setting('zencaptcha-settings-group', 'zenca-woo-login');
        register_setting('zencaptcha-settings-group', 'zenca-woo-register');
        register_setting('zencaptcha-settings-group', 'zenca-woo-resetpass');
        register_setting('zencaptcha-settings-group', 'zenca-woo-checkout');

        register_setting('zencaptcha-settings-group', 'zenca-elementor-activate');

        register_setting('zencaptcha-settings-group', 'zenca-wpforms-activate');

        register_setting('zencaptcha-settings-group', 'zenca-cf7-activate');
        register_setting('zencaptcha-settings-group', 'zenca-gravity-activate');
        //register_setting('zencaptcha-settings-group', 'zen-ninja-activate');
        register_setting('zencaptcha-settings-group', 'zenca-mailchimp-activate');
        register_setting('zencaptcha-settings-group', 'zenca-divilogin-activate');
        register_setting('zencaptcha-settings-group', 'zenca-divicontact-activate');
        register_setting('zencaptcha-settings-group', 'zenca-divicomment-activate');
        register_setting('zencaptcha-settings-group', 'zenca-diviemailoptin-activate');

        register_setting('zencaptcha-settings-group', 'zenca-avada-activate');
        register_setting('zencaptcha-settings-group', 'zenca-htmlforms-activate');
        register_setting('zencaptcha-settings-group', 'zenca-forminator-activate');
        register_setting('zencaptcha-settings-group', 'zenca-fluentform-activate');
        register_setting('zencaptcha-settings-group', 'zenca-profilebuilderlogin-activate');
        register_setting('zencaptcha-settings-group', 'zenca-profilebuilderregister-activate');
        register_setting('zencaptcha-settings-group', 'zenca-profilebuilderresetpass-activate');
        register_setting('zencaptcha-settings-group', 'zenca-ultimatememberlogin-activate');
        register_setting('zencaptcha-settings-group', 'zenca-ultimatememberregister-activate');
        register_setting('zencaptcha-settings-group', 'zenca-ultimatememberresetpass-activate');

        register_setting('zencaptcha-settings-group', 'zenca-wp-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-woo-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-elementor-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-wpforms-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-cf7-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-gravity-verifyemail');
        //register_setting('zencaptcha-settings-group', 'zen-ninja-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-mailchimp-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-divi-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-avada-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-htmlforms-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-forminator-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-fluentform-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-profilebuilder-verifyemail');
        register_setting('zencaptcha-settings-group', 'zenca-ultimatemember-verifyemail');

        register_setting('zencaptcha-settings-group', 'zenca_widgetstarts');

        //TODO: custom email blacklist register_setting('zencaptcha-settings-group', 'zenca_email_blacklist', array($this, 'cleanList'));
        register_setting('zencaptcha-settings-group', 'zenca_country_blacklist', array($this, 'cleanListUppercase'));

        if (get_option('zenca_widgetstarts') === false) {
            update_option('zenca_widgetstarts', 'none');
        }

        /*TODO: custom email blacklistif (get_option('email_blacklist') === false) {
            ZENCAPTCHA_MAIN::$instance->set_email_blacklist_default();
        }*/
        if (get_option('zenca_country_blacklist') === false) {
            ZENCAPTCHA_MAIN::$instance->set_country_blacklist_default();
        }
    }

    public function zenca_admin_notice_no_keys() {
            ...
            <p>
                <?php
                    printf(wp_kses_post(__(
                        ...
                        $url,
                        esc_url(ZENCA_REGISTRATION_URL)
                    );
                ?>
            </p>
        </div>
    ...
    public function widget_lang() {
        ...
        if ( $lang != "auto" ) {
            if ( ! array_key_exists($lang, ZENCA_WIDGET_LANGUAGES) ) {
                $lang = "auto";
            }
        }
        return $lang;
