...
if ( ! defined( 'ABSPATH' ) ) exit;
    ...
    //v1:if ( ! function_exists( 'zenca_verify_post' ) ) {
        function zenca_verify_post($useremail) {

            $postarray = zenca_retrieve_solution_email_from_post($useremail);
            $solution = $postarray['solution'];
            $useremail = $postarray['useremail'];

            $verification = zenca_solution_verification($solution, $useremail);
            ...
            if (!empty($useremail) && ($verification["emailvalid"] === "invalid_email" || $verification["emailvalid"] === "disposable_email")) {
                ...
                $result= ZENCAPTCHA_MAIN::zenca_use_valid_email_Alert();
                $error_codes = $verification["message"];
                return apply_filters( 'zencaptcha_verify_request', $result, $error_codes );
            }
            ...
            if (stripos($cleaned_country_blacklist, $countrycode) !== false) {
                $result= ZENCAPTCHA_MAIN::zenca_no_access();
                $error_codes = $verification["message"];
                return apply_filters( 'zencaptcha_verify_request', $result, $error_codes );
            }
    ...
    function zenca_shortcode( $atts ) {
        $zencaptchainstance = ZENCAPTCHA_MAIN::$instance;
        return (string) apply_filters( 'zencap_zencaptchacaptcha_content', zenca_insert_html_widget( $zencaptchainstance ) );
    }

    function zenca_insert_html_widget($zencaptchainstance){   
        $sitekey = $zencaptchainstance->get_site_key();
        $lang = $zencaptchainstance->widget_lang();
        $addCallback=false;   //TODO: get this from options!
        $startcaptcha='none';
        //could use this instead: $startcaptcha = get_option('zenca_widgetstarts', 'none');
        if (get_option('zenca_widgetstarts')=="focus"){
            $startcaptcha='focus';
        }
        else if (get_option('zenca_widgetstarts')=="auto"){
            $startcaptcha='auto';
        }
    ...
    /*v1:<noscript>Enable Javascript for the website to work correctly.</noscript>*/


    function zenca_async_defer($tag, $handle, $src){
    ...
    function zenca_retrieve_solution_email_from_post($useremail=''){
        //v1: $postedSolution = $_POST['zenc-captcha-solution'];
        $postedSolution = isset($_POST['zenc-captcha-solution']) ?
        sanitize_text_field($_POST['zenc-captcha-solution']) :
        '';
    ...
    function zenca_retrieve_solution_from_post()
    {
        //v1: $postedSolution = $_POST['zenc-captcha-solution'];
        $postedSolution = isset($_POST['zenc-captcha-solution']) ?
        sanitize_text_field($_POST['zenc-captcha-solution']) :
    ...
    function zenca_keys_not_set_response(){
        return array(
            "success" => false,
    ...
    function zenca_correct_placeholder_response(){
        return array(
            "success" => true,
            "message" => 'valid',
    ...
    function zenca_solution_verification($solution, $email=""){
        $api_url = 'https://www.zencaptcha.com/captcha/siteverify';

        $zencaptchainstance = ZENCAPTCHA_MAIN::$instance;
        if (!$zencaptchainstance->are_keys_set()) {
            return zenca_keys_not_set_response();
        }
        ...
        if (is_wp_error($response)) {
            return zenca_correct_placeholder_response(); 
        }
    ...
    function zenca_wp_lostpass_post($errors){
